...
// Função para validar autenticação (seguindo padrão hub-backend)
...
rota.post('/login', async (req, res) => {
    try {
        const { email, senha } = req.body; // Receber dados direto, sem JWT
...
rota.post('/listar_formularios', async (req, res) => {
    try {
        ...
        return res.json(jsonMount(true, formularios, 'Formulários carregados com sucesso!'));
    } catch (error) {
...
rota.post('/buscar_formulario', async (req, res) => {
    try {
        ...
        const formulario = await execute.execute_get_formulario_by_id(auth.data.formulario_id);
        const perguntas = await execute.execute_listar_perguntas_formulario(auth.data.formulario_id);
...
rota.post('/criar_formulario', async (req, res) => {
    try {
        ...
        if (!auth.data.titulo || !auth.data.data_limite) {
            return res.json(jsonMount(false, {}, 'Título e data limite são obrigatórios!'));
        }
        ...
        return res.json(jsonMount(true, { formulario_id: resultado[0].id }, 'Formulário criado com sucesso!'));
    } catch (error) {
...
rota.post('/editar_formulario', async (req, res) => {
    try {
        ...
        if (!auth.data.formulario_id || !auth.data.titulo || !auth.data.data_limite) {
            return res.json(jsonMount(false, {}, 'ID, título e data limite são obrigatórios!'));
        }
...
rota.post('/criar_pergunta', async (req, res) => {
    try {
        ...
        if (!auth.data.formulario_id || !auth.data.titulo || !auth.data.tipo || !auth.data.ordem) {
            return res.json(jsonMount(false, {}, 'Formulário, título, tipo e ordem são obrigatórios!'));
        }
        ...
        return res.json(jsonMount(true, { pergunta_id: resultado[0].id }, 'Pergunta criada com sucesso!'));
    } catch (error) {
...
rota.post('/editar_pergunta', async (req, res) => {
    try {
        ...
        if (!auth.data.pergunta_id || !auth.data.titulo || !auth.data.tipo) {
            return res.json(jsonMount(false, {}, 'ID, título e tipo são obrigatórios!'));
        }
...
rota.post('/reordenar_pergunta', async (req, res) => {
    try {
        ...
        if (!auth.data.formulario_id || !auth.data.pergunta_id || !auth.data.nova_ordem) {
            return res.json(jsonMount(false, {}, 'Formulário, pergunta e nova ordem são obrigatórios!'));
        }
        
        await execute.execute_reordenar_perguntas(auth.data.formulario_id, auth.data.pergunta_id, auth.data.nova_ordem);
...
rota.post('/respostas_formulario', async (req, res) => {
    try {
        ...
        const respostas = await execute.execute_get_respostas_formulario(auth.data.formulario_id);
        return res.json(jsonMount(true, respostas, 'Respostas carregadas com sucesso!'));
    } catch (error) {
...
rota.post('/detalhes_resposta', async (req, res) => {
    try {
        ...
        const detalhes = await execute.execute_get_detalhes_resposta(auth.data.resposta_formulario_id);
        return res.json(jsonMount(true, detalhes, 'Detalhes carregados com sucesso!'));
    } catch (error) {
...
rota.post('/top_participantes', async (req, res) => {
    try {
        ...
        return res.json(jsonMount(true, top, 'Top participantes carregado com sucesso!'));
    } catch (error) {
...
rota.post('/estatisticas', async (req, res) => {
    try {
        ...
        return res.json(jsonMount(true, estatisticas[0] || {}, 'Estatísticas carregadas com sucesso!'));
    } catch (error) {
